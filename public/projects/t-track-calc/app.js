...
function coreMinutes(r, p) {
  ...
  const install = tops * r.install_m11 * 0.085 + p.number_of_tops * r.install_m30;
  const base = r.cad_layout_base_minutes * 0.38;
  const travel = p.mileage * r.template_mileage_rate;
  return Math.max(12, (base + cad + saw + splash + install + travel) * VARIANT);
}
